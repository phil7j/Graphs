...
def find_path(roomGraph):
    ...
    s.push(0)
    while s.size() > 0:
    # Pop top room off of stack
        # print("hi")
        print(visited)
        print(traversalPath)
        cur_room = s.pop()
        # print(f'current room: {cur_room}')
    #   if not in visited : add as a key to the 'visited' object, then add directions as value
        if cur_room not in visited:

            # Get all exits of current room, add as values of current visited key
            exits_array = player.currentRoom.getExits()
            new_room_exits = {}
        ...
        # If current room has an unknown room connected to an exit
        ...
        # Travel to first direction with '?' direction
                if value == '?':
                    player.travel(key)
            # - set prev_room to cur_room and push location to the stack
                    prev_room = cur_room
            # Check if prev_room still has '?' rooms, if so push it to stack first, then push new room to stack
                    # if '?' in visited[prev_room]:
                    #     s.push(prev_room)
                    s.push(player.currentRoom.id)
            # set direction_traveled to first '?' direction
                    direction_traveled = key
            # append direction_traveled to traversalPath
                    traversalPath.append(direction_traveled)
                    break

        # Otherwise if no question marks found, go back to previous room
        else:
            # Check if there are questions left in visited
            # print(visited)
            # print("hi")
            ...
            # Move backwards until you find room with questions
            # print(f"TEST, in ' else' block: {cur_room}")
            for direction in reversed(traversalPath):
                if direction == 's':
                    new_direction = "n"
                # If direction was north, then direction for south will be previous node
                if direction == 'n':
                    new_direction = 's'
                # If direction was east, then direction for west will be previous node
                if direction == 'e':
                    new_direction = 'w'
                # If direction was west, then direction for east will be previous node
                if direction == 'w':
                    new_direction = 'e'
                # Add new direction to traversal Path
                traversalPath.append(new_direction)
                direction_traveled = new_direction
                # MOVE!
